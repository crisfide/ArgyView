...
class BDActivos {
    companion object{
        ...
        fun inicializarListados(paneles : List<String>) {
            paneles.forEach { panel ->
                val jsonStr = CrearActivo.obtenerJsonPref(panel) ?: return@forEach

                val data = CrearActivo.jsonToData(jsonStr)
                var lista = data
                    .map { it["symbol"] as String }
                    .filter { !arr.contains(it) }

                if (panel=="Bonos") lista = lista.filter {
                            !it.endsWith("X")
                            && !it.endsWith("Y")
                            && !it.endsWith("Z")
                }

                mapa[panel]?.addAll(lista)
